...
class DroneController():
    def __init__(self,node):
        ...
        nyq = 0.5 * fs
        wc = fc / nyq
        self.sos = scipy.signal.butter(N=order, Wn=4, btype='lowpass', analog=False, output='sos',fs=fs)
        self.DRONE_WHYCON_POSE=[[],[],[]]
        self.whycon_sub = node.create_subscription(PoseArray,"/whycon/poses",self.whycon_poses_callback,1)
        self.filtered_pub = node.create_publisher(Pose, "/whycon/filtered_pose",1)
...
def main(args=None):
    ...
    node = rclpy.create_node('filter1')
