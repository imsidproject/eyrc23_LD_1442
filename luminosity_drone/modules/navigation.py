...
class Navigation():
    def __init__(self):
        ...
        #self.setpoints = np.array(((-1,-1,-1),(1,-1,self.ht),(2,-1,self.ht),(2,1,self.ht),(1,1,self.ht),(1,-1,self.ht)),np.float64)
        self.setpoints = np.array(((-1,-1,-1),(-2,-2,self.ht),(2,-2,self.ht),(2,2,self.ht),(-2,2,self.ht),(-2,-2,self.ht)),np.float64)
        self.stpt_i = -1
    ...
    def get_direction(self, x):
        ...
        if dist <= 0:
            print(self.setpoints[self.stpt_i+1])
            self.stpt_i += 1
            if self.stpt_i ==5:
                self.stpt_i =1
        return error, vel
    ...
    def calc_error(self, x, p, q):
        ...
        if l == 0:
            speed = max(min(self.c0, dist*self.c0/self.c1),-self.c0)
        else:
            speed = max(min(self.c0*self.c2/abs(l), self.c0, dist*self.c0/self.c1),-self.c0)
        print("speed",speed)
