...
if __name__ == '__main__':
    ...
    r = rospy.Rate(33)
    setpoints = np.array(
        ((4, 4, 20), (-4, 4, 20), (-4, -4, 20), (4, -4, 20)), dtype=np.float64)
    i = 0

    fly_to_coord(swift_drone, r, setpoints[0])

    while not rospy.is_shutdown():
        err, speed, vel, dist = direction.give_direction(
            swift_drone.drone_position, setpoints[i], setpoints[(i+1) % 4])
        swift_drone.pid(error=err, vel=vel)
        if dist <= 0:
            i = (i+1) % 4
        print("e=", err, "s", speed, "v=", vel, "d=", dist)
        r.sleep()
